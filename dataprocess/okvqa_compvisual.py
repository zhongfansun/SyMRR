...
def main():
    ...
    for _, single_sample in problems.items():
        if single_sample["split"] == output_split:
            ...
            query_item = (
                ...
                + f"\nExplanation: {pred}."
                + "\nSummarize the key visual information mentioned in the explanation that is related to this image and question. Provide the summary in one short phrase."
                + "\nSummary:"
            )
    ...
    save_path = (
        ...
        f"okvqa_{large_model_name}_{small_model_name}_compvisual_{output_split}.csv"
    )
